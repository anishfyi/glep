...
- Output parity target: `rg -n --no-heading --color=never --sort path --no-require-git`. glep's default output must byte-match that invocation on text-only corpora. (`--no-require-git` because glep's sweep sets `require_git(false)`: .gitignore applies even outside git repos, per the spec's works-anywhere requirement.)
...
File format, little-endian (v2, amended during execution: a u64 generation stamp ties postings to their manifest so torn pairs are detected at open):

```
[8B magic "GLEPPOST"] [u32 version = 2] [u32 n_trigrams] [u64 generation]
...
`write` takes the generation as a third argument; `Postings::generation()` exposes it. `Manifest` carries a matching `generation: u64`. `open_or_build` verifies main.generation == manifest.generation (mismatch: retry once for racing readers, then rebuild) and attaches delta.bin only when its generation matches (stale deltas are dropped silently).
...
Run: `cargo test --test cli`. Expected: all pass (Task 10 implemented the behavior; these tests pin it). Note (amended during execution): glob matching uses gitignore semantics via a build_glob helper: slash-free patterns get a `**/` prefix and literal_separator(true), so `*.rs` matches at any depth while `src/*.rs` stays scoped to direct children.
...
jobs:
  test:
    ...
    steps:
      ...
      - name: em-dash ban
        run: |
          ! grep -rnE $'\xe2\x80\x94|\xe2\x80\x93' --include='*' . --exclude-dir=target --exclude-dir=.git
```
...
- Known crash window (reviewed, accepted): a crash between update()'s delta.bin rename and manifest.bin rename leaves a delta whose new ids the stale manifest does not know. This degrades safely (is_live drops out-of-range ids) and self-heals on the next query, because check-on-query runs update() before candidates(), and the never-saved manifest still diffs against disk. No stale result is served through any CLI path.
